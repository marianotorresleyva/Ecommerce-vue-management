...
lightbox.addEventListener("click", () => {
    ...
    images.forEach((image) => {
        //get value for current image
        if (image.classList.contains("active")) {
            const classList = image.classList;
            divCurrentImage.style.backgroundImage = `url('../src/assets/image-product-${classList[1]}.jpg')`;
        }
    });
    ...
    const lightBoxImages = document
        .getElementById("lightbox-images")
    ...
    nextButton.addEventListener("click", () => {
        for (i = 0; i < lightBoxImages.length; i++) {
            ...
            if (
                ...
                +classList[1] !== lightBoxImages.length
            ) {
    ...
    function executeLightBoxButton(image, isNextButton) {
        ...
        if (isNextButton) {
            ...
            divCurrentImage.style.backgroundImage = `url('../src/assets/image-product-${image.nextElementSibling.classList[1]}.jpg')`;
        } else {
            image.previousElementSibling.appendChild(div);
            image.previousElementSibling.classList.add("active");
            divCurrentImage.style.backgroundImage = `url('../src/assets/image-product-${image.previousElementSibling.classList[1]}.jpg')`;
        }
    }
});
...
const imageListener = (images, current, isLightBox) => {
    images.forEach((image) => {
        image.addEventListener("click", () => {
            ...
            const classList = image.classList;
            current.style.backgroundImage = `url('../src/assets/image-product-${classList[1]}.jpg')`;
    ...
    if (event.target.classList.contains("plus")) {
        ...
        if (+unitsValue.innerText == 0) {
            ...
            if (cartBody.classList.contains("empty")) {
                ...
                <div class="cart-product-img" style="background-image: url('../src/assets/image-product-1-thumbnail.jpg')"></div>
                <div class="product-name-price">
                  <p>${productName.innerText}</p>
                  <p>${productPrice.innerText} 
                  ${
                      +unitsValue.innerText == 1
                          ? '<span class="change-units"></span>'
                          : `<span class="change-units">x <span class="current-unit">${
                                unitsValue.innerText
                            }</span> <span style="color: #1d2025; font-weight: 700;">${
                                +productPrice.innerText.slice(1) *
                                +unitsValue.innerText
                            }.00</span></span>`
                  }</p>
                </div>
                <img src="../src/assets/icon-delete.svg" alt="icon-delete" id="delete">
              </div>
              ...
              <button class="checkout" onclick=" checkout()">Checkout</button>`;
                ...
                changeUnits.innerHTML = `x <span class="current-unit">${
                    unitsValue.innerText
                }</span> <span style="color: #1d2025; font-weight: 700;">${
                    +productPrice.innerText.slice(1) * +unitsValue.innerText
                }.00</span>`;
                changeCart();
            } else {
